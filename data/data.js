...
export const projectFrames = [
    {
        ...
        "<p>Let's walk through the updates we're seeing here, starting at the top of the page with the omni navigation. We now see the login button with a more distinct button shape. We've swapped out the pottery photo with a group of students (though it should be noted that any photo can live in this space, and the default would be either a slideshow, or a single video). Additionally, the tagline text is now on two lines instead of three.</p><p>Moving along it should be noticable that type faces have been updated globally, with Montserrat as our body copy typeface, while we've landed on Noto Serif for our larger headings, along with these no longer being italicized. We should also notice the color palette update, with the biggest note here that instances of the light chartruese color (nicer name for the color) have been removed and replaced with a cooler green. Continuing down the page past the large image links (we'll come back to to those on the next slide), I want highlight the \"Happening Around Westminster\" area. There was some discussion on the content that would go here, and what I'm highlighting here is just how flexible we can be with the content. The shown example here sees a highlighted news story, while on the side we have <em>both</em> a short event list and more news headlines. This represents a situation that has likely the most content one could reasonably expect in this area, however you're not married to having this much. We'll talk again and more about your options in this area.</p>",
    },
    {
        id:2,
        img:"/images/hp2.jpg",
        ...
        "<p>More updates to walkthrough here. Our main navigation dropdown now has an look that feels more attached to the item that's being interacted with, along with it being in a darker color as requested. Scrolling down the image links, we see an updated hovered item without the green overlay, and instead a very pale gold. I've also added an image zoom to this effect, though this will be more apparent once the site has been built.",
    },
   /*  {
        id:3,
        img:"/images/hp3.jpg",
